...
\noindent
expresses these algebraic quantities in terms of radicals, proving
\[ f_n = \frac{\left(12\sqrt{2}+17\right)^n}{\sqrt{(\pi n)^3}}\left(\frac{\sqrt{12+17/\sqrt{2}}}{4} + O\left(\frac{1}{n}\right)\right). \]
Of course, it is not always possible (or, even if possible, useful) to write the algebraic numbers appearing in asymptotics for general multivariate diagonals in radicals. 
\end{example}
...
\begin{enumerate}
    ...
    \item[2.] $H$ and all of its partial derivatives do not simultaneously vanish, so that all poles of $F$ are simple and $\sing$ forms a manifold,
    \item[3.] the smooth critical point system~\eqref{eq:CP} has a finite number of solutions, at least one of which is minimal, and
    \item[4.] the explicit matrix $\hes=\hes(\ww)$ defined by~\eqref{eq:hes} below is nonsingular at all minimal critical points $\ww$.
\end{enumerate}
...
\end{enumerate}
however it is unknown whether combinatorality is decidable, even for univariate rational functions, \textbf{so the user must verify this before running the algorithm} in order to guarantee that the output is correct. Table \ref{tab:parameters} summarizes the possible input parameters of the function \code{diagonal_asy}.
...
\begin{tabular}{ |p{0.18\linewidth}< {\centering} |p{0.28\linewidth}< {\centering} |p{0.34\linewidth} < {\centering}|}
 \hline 
...
\noindent
where now the leading constant is an algebraic number of degree 10 and the base of the exponentially growing term is an algebraic number of degree 5.
\end{example}
